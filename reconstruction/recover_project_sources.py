...
def is_text_candidate(record: InventoryRecord, blob: bytes) -> bool:
    ...
    if (
        ...
        and basename not in TEXT_BASENAMES
        and record.size > 100_000
    ):
        return False
...
def safe_relative_path(name: str) -> Path:
    ...
    if parts and parts[0] == "KIMI_WORKSPACE_RAW":
        parts = parts[1:]
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Recover verifiable folder-0 project sources from preserved raw bytes"
    )
