# DigitalOcean Console Bootstrap Fallback

Use this only when SSH from Codex is unavailable. Run it in the DigitalOcean web Console as `root` on droplet `134.199.144.115`.

The script:

- installs a Codex public key into `authorized_keys`
- installs Docker and Compose
- clones/pulls `rafsof22-lgtm/hub`
- creates `deployment/runtime-scaffold-pack/.env.production` with generated secrets
- starts the stack
- verifies local endpoints
- creates a dedicated GitHub Actions SSH key on the droplet and prints the private key for GitHub Secrets UI entry

Do not paste the generated private key into chat.
...
printf 'APP_DIR=%s\n' "$APP_DIR"
printf 'BASE_URL=%s\n' "$BASE_URL"
printf '\nDIGITALOCEAN_SSH_KEY value starts below. Add it to GitHub Secrets. Do not paste it into chat.\n'
printf '%s\n' '-----BEGIN DIGITALOCEAN_SSH_KEY-----'
cat "$GH_DEPLOY_KEY"
printf '%s\n' '-----END DIGITALOCEAN_SSH_KEY-----'
